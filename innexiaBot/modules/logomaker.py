...
async def lego(event):
 ...
 try:
    ...
    img.save(fname2, "png")
    await tbot.send_file(event.chat_id, fname2, caption="💕 Made By @i_am_albin_praveen Support @musicwithalby")
    if os.path.exists(fname2):
            os.remove(fname2)
 except Exception as e:
   await event.reply(f'Error Report @musicwithalby, {e}')
...
async def lego(event):
 ...
 try:
    ...
    fname2 = "LogoByInnexia.png"
    img.save(fname2, "png")
    await tbot.send_file(event.chat_id, fname2, caption="Made By @i_am_albin_praveen")
    if os.path.exists(fname2):
            os.remove(fname2)
 except Exception as e:
   await event.reply(f'Error Report @musicwithalby, {e}')
